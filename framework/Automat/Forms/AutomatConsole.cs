...
{
    public partial class AutomatConsole : Form
    {
        ...
        public AutomatConsole()
        {
            ...
            castToolStripMenuItem.Enabled = false;
        }
        ...
        protected void TradeAutonomously()
        {
            ...
            // Step: How to keep this running for a particular amount of time? The whirl is the answer (2022).
        }

        #region Deep learning elements for Encog
        ...
        {
            MessageBox.Show(@"Cartheur presents: Soft agent, version " + SharedFunctions.ApplicationVersion + @". Copyright 2008-2017 Cartheur Robotics, spol. s r.o., All rights reserved.", @"About this program", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
        }
        private void licenseToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MessageBox.Show(@"See if it will help administer change.", @"License", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
        }
        ...
        private void loadFileDataButton_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            sampleSizeLabel.Text = HardCoreAgent.LoadLocalData(PathToXms) + Environment.NewLine;
            if (AgentCore.LocalDataLoaded)
            {
                dataStateLabel.Text = @"Data from local file is in memory.";
                AeonOutput.Text += @"I have loaded data from the local file source." + Environment.NewLine;
            }
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            AeonOutput.Text += HardCoreAgent.ProcessMetricalData(int.Parse(numberOfCandlesBox.Text)) + Environment.NewLine;
            Cursor.Current = Cursors.Default;
        }
        private void trainSolverButton_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            //if (AgentCore.LocalDataLoaded)
            //    AeonOutput.Text += HardCoreAgent.TrainSolverNetwork() + Environment.NewLine;
            //else
                AeonOutput.Text += @"Load a dataset first." + Environment.NewLine;
            Cursor.Current = Cursors.Default;
        }
        private void trainForecastButton_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            //if (AgentCore.LocalDataLoaded)
            //    AeonOutput.Text += HardCoreAgent.TrainForecastNetwork() + Environment.NewLine;
            //else
                AeonOutput.Text += @"Load a dataset first." + Environment.NewLine;
            Cursor.Current = Cursors.Default;
        }
        private void castSolutionButton_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            //if (AgentCore.LocalDataLoaded && AgentCore.SolverTrainingDone)
                //AeonOutput.Text += HardCoreAgent.CastSolution() + Environment.NewLine;
            //else
                AeonOutput.Text += @"Load a dataset and do some training before trying to find a solution." + Environment.NewLine;
            Cursor.Current = Cursors.Default;
        }
        private void encogTradeOnceButton_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            TradeAutonomously();
            Cursor.Current = Cursors.Default;
        }
        private void boagaphishTradeOnceButton_Click(object sender, EventArgs e)
        {
            AeonOutput.Text += HardCoreAgent.ProspectTrade() + Environment.NewLine;
        }
        private void portfolioManagerButton_Click(object sender, EventArgs e)
        {
            if (PortfolioManager.Instance == false)
            {
                var form = new PortfolioManager(this);
                form.Show(this);
                PortfolioManager.Instance = true;
            }
            else if (PortfolioManager.Instance)
            {
                // Do nothing.
            }
        }
        private void displayArrayButton_Click(object sender, EventArgs e)
        {
            ...
            {
                //var form = new ChartForm(this, ChartProperties.ChartType.Candles);
                //form.Show(this);
                //ChartForm.Instance = true;
            }
        ...
        {
            if (PlaceOrder.Instance == false)
            {
                var form = new PlaceOrder(this);
                form.Show(this);
                PlaceOrder.Instance = true;
            }
            else if (PlaceOrder.Instance)
            {
                // Do nothing.
            }
        }
        ...
        private void forecastTrainToolStripMenuItem_Click(object sender, EventArgs e)
        {
            //if (AgentCore.LocalDataLoaded && AgentCore.ForecastTrainingDone)
            //    AeonOutput.Text += HardCoreAgent.TrainForecastNetwork() + Environment.NewLine;
            //else
                AeonOutput.Text += @"Load a dataset first." + Environment.NewLine;

        }
        private void solverTrainToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AgentCore.LocalDataLoaded)
            {
                //AeonOutput.Text += HardCoreAgent.TrainSolverNetwork() + Environment.NewLine;
                castToolStripMenuItem.Enabled = true;
                Refresh();
            }
            else
                AeonOutput.Text += @"Load a dataset first." + Environment.NewLine;
        }
        private void solverNetworkLoadToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }
        private void forecastNetworkLoadToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }
        private void predictorNetworkLoadToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        {
            HardCoreAgent.LoadLocalData(PathToXms);
        }
        private void liveSourceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            HardCoreAgent.ProcessMetricalData(int.Parse(numberOfCandlesBox.Text));
        }
        #endregion

        private void generateToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (HardCoreAgent.RetrieveContainerData(containerNameComboBox.SelectedItem.ToString(), numberOfCandlesBox.Text));
            AeonOutput.Text = "Metric file retrieved and saved successfully.";
        }
