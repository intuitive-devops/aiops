...
public abstract class NoiseBase<GradientType>
{
    // The function we use to smooth the interpolation between the different corners of the cube. With a linear interpolation we'll get hard edges.
   ...
   138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180};
    ...
    // Function that performs the dot product (inner product) of two 3D vectors where one of the vectors is stored in the GradientType type.
    private Func<GradientType, double, double, double, double> Dot;

    protected NoiseBase(GradientType[] gradients, Func<GradientType, double, double, double, double> dot, Func<double, double> smoothingFunction)
    {
    ...
    public double Noise(double x, double y = 0.5d, double z = 0.5d)
    {
		...
		**/
        ...
        // calculate smoothed x, y and z values. These are used to get a smoother interpolation between the dot products of the gradients and local coords
    ...
    /// <param name="y">The y.</param>
    /// <param name="z">The z.</param>
    /// <param name="numOctaves">The number octaves.</param>
    /// <param name="lacunarity">The lacunarity.</param>
    /// <param name="persistence">The persistence.</param>
    /// <returns></returns>
    public double NoiseOctaves(double x, double y, double z = 0.5d, int numOctaves = 6, double lacunarity = 2d, double persistence = 0.5d)
    {
    ...
    /// <param name="y">The y.</param>
    /// <param name="z">The z.</param>
    /// <param name="tileRegion">The tile region.</param>
    /// <param name="numOctaves">The number octaves.</param>
    /// <param name="lacunarity">The lacunarity.</param>
    /// <param name="persistence">The persistence.</param>
    /// <returns></returns>
    public double NoiseTiledOctaves(double x, double y, double z, int tileRegion = 2, int numOctaves = 6, double lacunarity = 2d, double persistence = 0.5d)
    {
        double noiseValue = 0d;
        double amp = 1d;
        double freq = 1d;
    ...
    // use a different permutationTable then the provided default. This will change the look of the noise
    public void SetPermutationTable(int[] newPermutationTable)
    {
        // make sure the new PT has Length = 2^N (this property is used in the Noise function)
    ...
    /// <summary>
    /// Takes a value in the range 0..1 and returns an s-curve in the range 0..1. This is a recommended replacement for the original 3t^2 - 2t^3 from https://mrl.nyu.edu/~perlin/paper445.pdf.
    /// </summary>
    /// <param name="value">The value.</param>
    /// <returns></returns>
    protected static double SmoothToSCurve(double value)
    {
