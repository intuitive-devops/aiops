...
namespace SweetPolynomial
{
    public class Matrix
    {
        ...
        /// <summary>
        /// Creates real matrix from string, e.g. "1,0;0,1" gives the 2 by 2 identity matrix. Not fast, but easy to use, if matrices are to be entered by hand or read from text files.
        /// </summary>
        ...
        public static Matrix DFS(Matrix adjacence_matrix, int root)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                ...
                for (int j = 1; j <= n; j++)
                    if (adjacence_matrix[i, j].Real != 0 && adjacence_matrix[i, j].Imaginary != double.PositiveInfinity)
                        A[i].Add(j);
            }
        ...
        public static Matrix BFS(Matrix adjacence_matrix, int root)
        {
            ...
            for (int i = 1; i <= n; i++)
            {
                ...
                for (int j = 1; j <= n; j++)
                    if (adjacence_matrix[i, j].Real != 0 && adjacence_matrix[i, j].Real != double.PositiveInfinity)
                        A[i].Add(j);
            }
        ...
        public Matrix Real()
        {
            ...
            for (int i = 1; i <= rowCount; i++)
                for (int j = 1; j <= columnCount; j++)
                    M[i, j] = new Complex(this[i, j].Real);
        ...
        public Matrix Im()
        {
            ...
            for (int i = 1; i <= rowCount; i++)
                for (int j = 1; j <= columnCount; j++)
                    M[i, j] = new Complex(this[i, j].Imaginary);
        ...
        public double FrobeniusNorm()
        {
            ...
            for (int i = 1; i <= n; i++)
                for (int j = 1; j <= n; j++)
                    buf += (this[i, j] * Complex.Conj(this[i, j])).Real;
            ...
            for (int i = 1; i <= rowCount; i++)
            {
                for (int j = 1; j <= columnCount; j++)
                {
                    ...
                    if (buf.Real == double.PositiveInfinity || buf.Real == double.NegativeInfinity
                        || buf.Imaginary == double.PositiveInfinity || buf.Imaginary == double.NegativeInfinity)
                        s += "oo";
                    else if (buf.Real == double.NaN || buf.Imaginary == double.NaN)
                        s += "?";
                    else
                        s += buf.ToString();
